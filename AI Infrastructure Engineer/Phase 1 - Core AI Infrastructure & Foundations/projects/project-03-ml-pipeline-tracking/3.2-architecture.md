...
This document details the system design, network topography, and technical integrations that power the end-to-end Machine Learning Pipeline. It covers how data version control (DVC), experiment tracking (MLflow), and workflow automation (Apache Airflow) are organized into a unified, secure, and reproducible platform.
...
4. [MLflow Architecture](#mlflow-architecture)
5. [Airflow Architecture](#airflow-architecture)
...
graph TD
    ...
    classDef stage fill:#f9ebd2,stroke:#d35400,stroke-width:2px;
    classDef storage fill:#d5f5e3,stroke:#27ae60,stroke-width:2px;
    classDef tracking fill:#ebdef0,stroke:#8e44ad,stroke-width:2px;
    classDef orchestrator fill:#fcf3cf,stroke:#f1c40f,stroke-width:2px;

    subgraph Orchestration ["Workflow Automation"]
        Airflow["Apache Airflow (Task Scheduling, Retries & Monitoring)"]:::orchestrator
    end

    subgraph Inputs ["Data Ingest"]
        Sources["Data Sources (CSV, REST APIs, Databases)"]:::datasource
        Ingest["Data Ingestion & Validation (DataIngestion, Great Expectations)"]:::stage
    end

    subgraph Storage ["Data Version Control"]
        DVC["DVC Data Versioning (MinIO Remote, Git Metadata)"]:::storage
        Versioning["Data Versioning (Raw & Processed Data)"]:::storage
    end

    subgraph Processing ["Feature Engineering"]
        Prep["Data Preprocessing (Clean, Feature Eng, Splits)"]:::stage
        DB["Feature Store (PostgreSQL structured features)"]:::storage
    end

    subgraph Training ["Model Dev & Experiment Tracking"]
        Train["Model Training (PyTorch, Hyperparameters)"]:::stage
        MLflow["MLflow Tracking Server (Params & Loss Metrics)"]:::tracking
        Artifacts["Model Artifacts (PyTorch weights, ONNX, Plots)"]:::tracking
    end

    subgraph Evaluation ["Validation & Deployment"]
        Eval["Model Evaluation (Test Metrics, Confusion Matrix)"]:::stage
        Registry["MLflow Model Registry (Versions, Lifecycle Stages)"]:::tracking
    end
...
graph TD
    ...
    subgraph MLflowLayer ["MLflow Platform Layer"]
...
```mermaid
flowchart TD
    Raw["Raw Data"] --> Dedup["Remove Duplicates"]
    Dedup --> MissVal["Handle Missing Values"]
    MissVal --> Encode["Encode Labels"]
    Encode --> Split["Stratified Split"]
    Split --> Save["Save Splits + Artifacts"]
```

**Artifacts Generated:**

These artifacts are serialized objects saved after preprocessing so the **exact same transformations** can be applied to new data during inference.

| Artifact | Format | How It's Formed | Used For |
|----------|--------|-----------------|----------|
| `label_encoder.pkl` | Pickle (`.pkl`) | `sklearn.preprocessing.LabelEncoder` — maps string labels (e.g. "cat", "dog") to integer IDs (0, 1). Saved via `pickle.dump(encoder, file)`. | Converting model output integers back to human-readable class names at inference time. |
| `scaler.pkl` | Pickle (`.pkl`) | `sklearn.preprocessing.StandardScaler` — computes `mean` & `std` of training features, then normalizes: `(x - mean) / std`. Saved via `pickle.dump(scaler, file)`. | Applying identical normalization to production data so the model receives inputs in the same distribution it was trained on. |
| `preprocessing_config.json` | JSON | Python dict written with `json.dump()`. Stores metadata like: column names, feature order, split ratios, random seed, and any encoding mappings. | Reproducing the exact same preprocessing steps in a new environment without re-running the code. |

**How `.pkl` files work:** Python's `pickle` module **serializes** (converts) in-memory Python objects into a binary file format. When you `pickle.dump(encoder, file)`, it writes the object's entire state (all attributes, fitted parameters, class mappings) to disk. At inference time, `pickle.load(file)` reconstructs an identical object ready to transform new data — no re-training needed.
